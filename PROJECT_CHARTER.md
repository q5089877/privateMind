...
* **交互哲學（避風港注水下錨）**：
  ...
  * **全螢幕定錨注水（Full-Screen Ballast Water）**：按住 ⚓ 不放時，整面螢幕自底端湧升半透明港灣深邃潮水，將輸入框與所有文字雜訊物理性遮蔽；螢幕中央浮現緩緩下沉的巨大 ⚓ 透光剪影。
  * **節奏微震與低頻衝擊**：注水期間（0% ➔ 99%）每累積 20% 觸發一次 `55ms` 實體微震與 85Hz 物理低頻音。
  * **滿載觸底咬合（100%）**：潮水淹過頂部瞬間（約 2.2 秒），錨定海床，觸發重磅 `docking` 機械咬合微震（`[80ms, 40ms, 110ms]`），提示「已定錨 · 風浪平息，前額葉重置完成」，隨後潮水安靜散去，畫面重回清明。
  * **中途釋放容錯**：中途放開時潮水如重力般平滑退回底部，完全不阻礙正常操作。
* **無文字停靠**：點滿 3 次後浮現淡色統計與轉化錨點 `已定錨 N 次 · 想說的話，再點這裡`。即使完全不輸入任何字，純下錨亦屬合法完整的生理停靠紀錄。
