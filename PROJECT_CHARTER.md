...
## 00｜Level 0 消波微震（戳戳樂／能量充能條／物理出口）
* **定位**：純生理觸覺出口，解決急性混亂（心跳快、大腦超載、連字都不想面對）的零文字阻抗門檻。
* **交互原則**：極克制、無文字壓力、非遊戲化。
* **UI 實體規格**：
  * **零佈局偏移（Zero Layout Shift）**：尺寸嚴格固定為 `w-[144px]`、`h-11`（44px），無論操作狀態文字始終固定為 `● 消波微震`，絕不隨點擊縮放抖動。
  * **能量充能條（Energy Gauge Bar）**：按鈕內部底層內建注能水位層（`bg-accent/20`），按住時 2.2 秒平滑累積 0% ➔ 100%，右側以等寬數值顯示進度百分比。
* **雙模態觸覺回饋**：
  * **單擊輕戳（Tap）**：每次觸發 `55ms unlatch` 實體微震 + 跨感官低頻衝擊音，釋放急性動能。
  * **久按充能（Hold-to-Vent）**：按住不放時平滑注能，每累積 20% 水位觸發一次節奏微震；充滿 100% 觸發深度 `docking` 咬合微震（`[80ms, 40ms, 110ms]`），提示大腦降速落地，計數器 +1 並平滑歸零。
* **無文字停靠**：點滿 3 次後浮現淡色統計與轉化錨點 `已消波 N 次 · 想說的話，再點這裡`。即使完全不輸入任何字，純戳戳／充能亦屬合法完整的生理停靠紀錄。
