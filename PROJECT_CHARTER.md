...
## 03｜停靠完成狀態與時序
* **停靠時序 (Timing Sequence)**：
  1. `0.0s`：使用者點擊「停靠」 $\rightarrow$ Input Blur，Keyboard Dismiss。
  2. `0.0s ～ 0.25s`：原文字離開輸入區，Entry 落定於卡片。
  3. `0.25s ～ 0.8s`：「放在這裡了。」淡入。
  4. `0.8s ～ 3.0s`：安靜停留（0.8～3 秒保持安靜，不主動催促使用者）。卡片永久留下作為確認。
  5. `3.0s ～ 3.8s`：「放在這裡了。」提示字樣淡出，回到極致安靜。
* **中斷規則 (Interrupt Rule)**：
  若使用者在動畫期間點擊「＋ 接著說……」，提示立即淡出，輸入框重新 Focus 並喚起鍵盤。
* **核心原則**：鍵盤收起、Focus 解除、提示短暫存在、卡片留下、使用者可離開亦可主動繼續。
