...
## 00｜Level 0 消波微震（戳戳樂／物理出口）
* **定位**：純生理觸覺出口，解決急性混亂（心跳快、大腦超載、連字都不想面對）的零文字阻抗門檻。
* **交互原則**：極克制、無文字壓力、非遊戲化。
* **按鈕設計**：右上角 44px（`h-11`）標準無障礙觸控藥丸按鈕，單手可戳，按住時具備共振光圈擴散動效。
* **字元序列**：`['停', '先別想', '呼', '可以', '隨它']`（極短、極中性，嚴禁心靈雞湯如「我知道」或解釋身心狀態）。
* **雙模態觸覺回饋**：
  * **單擊狂戳（Tap）**：每次觸發 `15ms unlatch` 脫扣微震，釋放急性動能。
  * **久按連震（Hold-to-Vent）**：按住不放觸發心律接地連震（每 170ms 震動一次並平滑輪轉字詞）；按住達 2.5 秒觸發深度 `docking` 沉降微震，提示大腦降速；放開即停。
* **無文字停靠**：點滿 3 次後浮現淡色統計與轉化錨點 `已消波 N 次 · 想說的話，再點這裡`。即使完全不輸入任何字，純戳戳也是合法完整的生理停靠紀錄。
