 ...
 *   G2  Total span: 7d ≤ Δt ≤ 60d
 *   G3  At least one consecutive pair separated by ≥ 3 days
 *       (rejects single-wave bursts while allowing same-day doubles)
 *   G4  Content length ≥ 10 Chinese characters (rejects pure noise)
 */
export const patternEligibility = (moments: Moment[]): Moment[] | null => {
    ...
    const valid = pool.filter(m => m.content.trim().length >= 10);
    ...
    if (span < 7 * DAY || span > 60 * DAY) return null;

    // G3: at least one gap of ≥ 3 days anywhere in the pool
    const hasBreak = valid.some(
        (m, i) => i > 0 && m.createdAt - valid[i - 1].createdAt >= 3 * DAY
    );
