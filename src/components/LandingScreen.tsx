...
export const LandingScreen: React.FC<Props> = ({ session, closure, onReturnToChat, onSaveAndReturn }) => {
  ...
  return <div className="w-full max-w-[560px] min-h-[calc(100vh-104px)] py-5 sm:py-8">
    ...
    <main className="pt-10 sm:pt-14">
      <div className="flex items-center gap-2 text-sm font-medium text-accent"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-accent/10"><Waves size={15}/></span>暫時停在這裡</div>
      <h1 className="mt-5 text-[32px] font-medium tracking-[-0.05em] text-ink sm:text-[40px]">今天先收在這裡。</h1>
      <p className="mt-3 max-w-md text-[16px] leading-relaxed text-ink-secondary">這不是結論；只是把今晚該想的到此打住。</p>
      ...
      <section className="mt-10 rounded-[30px] border border-border-base bg-surface p-6 shadow-[0_10px_28px_rgba(47,70,54,0.08)]">
        <div className="rounded-2xl bg-surface-subtle px-4 py-4">
          <p className="flex items-center gap-1.5 text-xs font-medium text-accent"><Waves size={14}/>這次先帶走</p>
          <p className="mt-2 text-[17px] leading-[1.8] text-ink">{closure.takeaway}</p>
        </div>
        <div className="mt-5 border-l-2 border-accent/45 pl-4">
          <p className="text-xs font-medium text-accent">留在明天看</p>
          <p className="mt-2 text-[16px] leading-[1.75] text-ink-secondary">{closure.unresolved}</p>
          {closure.resumeAnchor && <p className="mt-3.5 border-t border-border-base/70 pt-2.5 text-xs text-ink-muted leading-relaxed">下次若要接著談，可以從「{closure.resumeAnchor}」開始，不擴大戰線。</p>}
        </div>
      </section>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-2">
        <button onClick={() => void handleSave()} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-accent px-6 text-sm font-medium text-white shadow-[0_6px_14px_rgba(47,91,71,0.2)] hover:bg-accent-hover transition-colors">
          <Lock size={15}/>封存並回到現在
        </button>
        <button onClick={onReturnToChat} className="inline-flex min-h-11 text-sm text-ink-secondary hover:text-ink">
          還想再說一點
        </button>
      </div>
      <p className="mt-5 text-xs leading-relaxed text-ink-muted">選擇回到現在後，這份邊界封存才會和本次對話一起保存。</p>
    </main>
  </div>;
};
