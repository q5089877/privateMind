...
export const BackupScreen: React.FC<Props> = ({ getOverview, onExport, onImport, onClose }) => {
  ...
  const exportBackup = async () => { await onExport(); await refresh(); setMessageKind('success'); setMessage('備份檔已下載。請放在自己找得到的安全位置。'); };
  const chooseImport = () => inputRef.current?.click();
  const importBackup = async (file?: File) => {
    if (!file) return;
    try { await onImport(await file.text()); await refresh(); setMessageKind('success'); setMessage('已合併匯入備份；現有資料沒有被覆蓋。'); }
    catch (error) { setMessageKind('error'); setMessage(error instanceof Error ? error.message : '無法讀取這份備份檔。'); }
  ...
  return <div className="w-full max-w-[560px] min-h-[calc(100vh-104px)] py-5 sm:py-8">
    ...
    <main className="pt-12 sm:pt-16">
      ...
      <section className="mt-5 rounded-3xl border border-border-base bg-surface-subtle p-5"><p className="text-sm font-medium text-ink">這份備份會帶走</p><div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm text-ink-secondary"><p>原始念頭 <span className="text-ink-muted">{overview?.moments ?? '–'} 則</span></p><p>停靠對話 <span className="text-ink-muted">{overview?.sessions ?? '–'} 次</span></p><p>對話回合 <span className="text-ink-muted">{overview?.turns ?? '–'} 則</span></p><p>暫時收束 <span className="text-ink-muted">{overview?.closures ?? '–'} 份</span></p><p>跨時間連線 <span className="text-ink-muted">{overview?.lines ?? '–'} 條</span></p><p>定錨互動 <span className="text-ink-muted">{overview?.anchorEvents ?? '–'} 筆</span></p><p className="col-span-2">連線的確認、拒絕與暫緩紀錄 <span className="text-ink-muted">{overview?.decisions ?? '–'} 筆</span></p></div><p className="mt-4 text-xs leading-relaxed text-ink-muted">匯入時會先驗證格式，再與這台裝置的資料合併；既有內容不會被覆蓋。</p></section>
      ...
      {message && <p role={messageKind === 'error' ? 'alert' : 'status'} className={`mt-5 rounded-2xl border p-4 text-sm leading-relaxed ${messageKind === 'error' ? 'border-red-200 bg-red-50 text-red-700' : 'border-border-base bg-surface-subtle text-ink-secondary'}`}>{message}</p>}
