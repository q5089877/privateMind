...
import { DiscoveryInsight, GeminiProxyClient } from '../logic/geminiProxyClient';
...
export const DiscoveryScreen: React.FC<Props> = ({ getMoments, onClose, onKeepLine }) => {
  ...
  useEffect(() => {
    ...
    void getMoments().then(async moments => {
      ...
      const result = await GeminiProxyClient.findDiscoveryInsight(indexed);
      if (!alive) return;
      setInsight(result ? { ...result, momentIds: result.momentIds.map(id => indexed.find(item => item.id === id)?.sourceId).filter((id): id is string => Boolean(id)) } : null);
      setLoading(false);
    });
    return () => { alive = false; };
  }, [getMoments]);
  ...
  return <div className="w-full max-w-[560px] min-h-[calc(100vh-104px)] py-5 sm:py-8">
    ...
    <main className="pt-12 sm:pt-16">
      ...
      {loading ? <div className="mt-14 rounded-3xl border border-border-base bg-surface-subtle p-6"><p className="text-[16px] leading-relaxed text-ink-secondary">正在從近期記錄裡找一段值得回看的地方…</p><p className="mt-3 text-xs leading-relaxed text-ink-muted">只在你主動點選後進行。</p></div> : insight ? <section className="mt-12 rounded-[28px] border border-border-base bg-surface p-6 shadow-[0_5px_16px_rgba(47,70,54,0.08)]"><p className="text-sm font-medium text-accent">看得見的一個角度</p><p className="mt-4 text-[18px] leading-[1.85] text-ink">{insight.angle}</p><div className="mt-6 space-y-3 border-l border-accent/30 pl-4">{insight.evidence.map((item, index) => <p key={`${item.date}-${index}`} className="text-sm leading-relaxed text-ink-secondary"><span className="text-xs text-ink-muted">{item.date}</span><br/>「{item.phrase}」</p>)}</div><p className="mt-6 border-t border-border-base pt-5 text-[16px] leading-[1.75] text-ink">{insight.question}</p><div className="mt-6 flex flex-wrap gap-x-5 gap-y-2"><button onClick={() => void onKeepLine(insight.momentIds)} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-accent px-5 text-sm font-medium text-white"><Link2 size={15}/>把這幾段留在一起</button><button onClick={onClose} className="min-h-11 text-sm text-ink-secondary hover:text-ink">先收起</button></div></section> : <section className="mt-14 rounded-3xl border border-border-base bg-surface-subtle p-6"><p className="text-[17px] leading-relaxed text-ink">目前沒有一段夠可靠的新角度。</p><p className="mt-3 text-sm leading-relaxed text-ink-secondary">先讓它們各自在時間流裡待著；多留一些不同日子的片段後，再回來看看。</p><button onClick={onClose} className="mt-6 text-sm font-medium text-accent">回到現在</button></section>}
    </main>
  </div>;
};
