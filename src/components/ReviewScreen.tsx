...
export const ReviewScreen: React.FC<Props> = ({ onClose, getMoments, getSessions, onOpenSession, onRequestReading, onOpenBackup }) => {
  ...
  return <div className="w-full max-w-[640px] pb-20">
    <header className="flex h-12 items-center justify-between"><button onClick={onClose} className="flex min-h-11 items-center gap-1.5 px-1 text-sm text-ink-secondary hover:text-ink"><ArrowLeft size={16}/>現在</button><button onClick={onOpenBackup} className="flex min-h-11 items-center gap-1.5 px-1 text-xs text-ink-muted hover:text-ink"><HardDrive size={15}/>資料與備份</button></header>
    <main>
      <div className="mt-10"><div className="flex items-center gap-2 text-accent"><Waves size={18}/><span className="text-sm font-medium">回看</span></div><h1 className="mt-5 text-[32px] font-medium tracking-[-0.05em] text-ink sm:text-[40px]">事情怎麼走到今天</h1><p className="mt-3 max-w-md text-[16px] leading-relaxed text-ink-secondary">每一次停靠都留在發生的位置。想回看時，AI 會從原文與日期裡找一個值得再看的地方。</p></div>

      <section className="mt-10 rounded-[28px] border border-accent/20 bg-accent/5 p-5"><div className="flex items-center gap-2 text-sm font-medium text-accent"><Sparkles size={16}/>回看最近留下的事</div><p className="mt-2 max-w-lg text-sm leading-relaxed text-ink-secondary">只在你現在主動點選後讀取跨時間原文。它不會建立群組，也不會替你做決定。</p>{readingState === 'loading' ? <p className="mt-5 text-sm text-ink-muted">正在從留下的時間與原話裡找一個可驗證的新角度…</p> : <button onClick={() => void requestReading()} className="mt-5 inline-flex min-h-11 items-center rounded-full bg-accent px-5 text-sm font-medium text-white shadow-sm">幫我回看</button>}
        {reading && <div className="mt-6 border-t border-accent/15 pt-5"><p className="text-sm font-medium text-accent">留下的軌跡</p><div className="mt-3 space-y-3 border-l border-accent/30 pl-4">{reading.evidence.map((item, index) => <p key={`${item.date}-${index}`} className="text-sm leading-relaxed text-ink-secondary"><span className="text-xs text-ink-muted">{item.date}</span><br/>「{item.phrase}」</p>)}</div><div className="mt-6"><p className="text-sm font-medium text-accent">看得見的變化</p><p className="mt-2 text-[17px] leading-[1.8] text-ink">{reading.angle}</p></div><div className="mt-6 border-l-2 border-accent/40 pl-4"><p className="text-sm font-medium text-accent">還可再看的地方</p><p className="mt-2 text-[16px] leading-[1.75] text-ink-secondary">{reading.unresolved}</p></div></div>}
        {readingState === 'empty' && <p className="mt-5 border-l-2 border-border-base pl-4 text-sm leading-relaxed text-ink-secondary">目前還沒有足夠跨時間、可驗證的原文。先讓它們留在時間流裡，之後再回來看看。</p>}
      </section>

      {moments.length === 0 ? <p className="mt-12 border-t border-border-base py-16 text-sm text-ink-muted">這裡還沒有留下任何事。</p> : <div className="mt-12 space-y-10">{(Object.entries(timeline) as Array<[string, TimelineItem[]]>).map(([date, items]) => <section key={date} className="relative border-l border-accent/25 pl-5"><span className="absolute -left-[5px] top-1.5 h-2.5 w-2.5 rounded-full bg-accent ring-4 ring-canvas"/><p className="text-sm text-ink-secondary">{date}</p><div className="mt-4 space-y-5">{items.map(item => {
