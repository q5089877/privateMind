...
export const ReviewScreen: React.FC<Props> = ({ onClose, getMoments, getSessions, getLines, onOpenSession, onOpenLine, onCreateManualLine, onOpenBackup }) => {
  ...
  // Lines are many-to-many. A session may touch several confirmed connections,
  // so show every matching line rather than hiding all but the first one.
  const relatedLines = (session: HarborSession) => lines.filter(line => session.momentIds.some(momentId => line.momentIds.includes(momentId)));
