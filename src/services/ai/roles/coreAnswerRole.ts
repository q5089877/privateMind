...
3. answer 是完整的「對應解說」，控制在 120–520 個中文字；請用 2–3 個自然段落分段，每段之間空一行，讓手機閱讀容易掃讀。不要使用 Markdown 標題、井號或長清單。逐段解釋前面的 quote 如何對應使用者原文。金剛經聚焦念頭與事實、四相、無所住與法尚應捨；道德經聚焦用力、控制、留白與順勢。經典只作為回答依據，不要把道德經寫成命令或泛泛的人生格言。
