...
export const landingRole = {
  create(turns: ConversationTurn[]): GeminiRoleRequest | null {
    ...
    const responseSchema = {
      type: 'OBJECT', properties: {
        takeaway: { type: 'STRING', description: '12 到 32 字；整理對話中已說明白的客觀事實。' },
        unresolved: { type: 'STRING', description: '8 到 20 字；只描述目前缺少的事實或不可控的外部變數。' },
        resumeAnchor: { type: 'STRING', description: '0 到 14 字；摘錄一個具體方向作為下次接點。' }
  ...
  read(raw: string): SessionClosureDraft | null {
    ...
    if (takeaway.length < 8 || takeaway.length > 32 || unresolved.length < 8 || unresolved.length > 20 || resumeAnchor.length > 14 || unresolved.includes('？') || unresolved.includes('?') || !unresolvedHasOneSentence || invalid(takeaway) || invalid(unresolved) || invalid(resumeAnchor)) return null;
