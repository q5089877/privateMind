...
export const presentFallback = (value: string) => {
  ...
  return `「${excerpt}」已經留下來。如果想沿著這句看，此刻最想讓人理解的是哪一部分？`;
};
...
export const presentRole = {
  create(current: string): GeminiRoleRequest {
    return {
      ...
      payload: {
        ...
        contents: [{ role: 'user', parts: [{ text: `使用者剛留下這一句：\n${current}\n\n請以繁體中文回覆 2 到 3 句、30 到 160 個中文字。只根據這一句話。第一句必須直接引用或重複其中一段 2 到 28 字的具體用語，準確映照使用者說出的狀態；第二句指出一個能從原文驗證、但使用者可能尚未說明白的新理解，若含推測必須用「也許」「可能」「像是」；最後一句只留一個貼著原話、可選擇回答的續談入口。不要把三句都寫成問題，也不要只是換句話說。不得提及過去紀錄、記憶、模式、重複或任何你未看見的內容。\n\n禁止診斷、心理標籤、建議、命令、空泛安慰、聲稱知道真正原因，也禁止使用「辛苦了」「這很正常」「真實的一刻」「一切正在運作」「允許自己」「先停下來」「休息一下」。不要替使用者下結論；保留今天不必想完的餘地。` }] }],
        generationConfig: { temperature: 0.45, maxOutputTokens: 220, responseMimeType: 'text/plain', thinkingConfig: FAST_THINKING_CONFIG }
      }
    };
  },
  ...
  read(raw: string, current: string): string {
    ...
    return text && text.length >= 30 && text.length <= 160 && grounded && !generic.some(phrase => text.includes(phrase)) ? text : presentFallback(current);
  }
};
