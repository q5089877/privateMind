...
export default function App() {
  ...
  const ask = async (content: string, previous: Turn[] = []) => {
    setLoading(true);
    try { setAnswer(await getCoreAnswer({ content, recentContext: previous.map(turn => turn.content).slice(-3), preferredLens: 'diamond_sutra' })); }
    finally { setLoading(false); }
  };
  const start = async () => {
    const content = text.trim();
    if (!content) return;
    setText(''); setTurns([{ role: 'user', content }]); setAnswer(null); setAlternative(null); setChat(true); await ask(content);
  };
  const continueChat = async () => {
    const content = text.trim();
    if (!content || loading) return;
    const previous = turns;
    setText(''); setTurns(current => [...current, { role: 'user', content }]); setAnswer(null); setAlternative(null); await ask(content, previous);
  };
  const askAlternative = async () => {
    ...
    try { setAlternative(await getCoreAnswer({ content: latest.content, recentContext: turns.map(turn => turn.content).slice(-3), preferredLens: 'i_ching' })); }
  ...
  if (chat) return <main className="mx-auto min-h-screen w-full max-w-[680px] px-4 py-5 sm:px-8">
    ...
    <section className="space-y-5">{turns.map((turn, index) => turn.role === 'user' ? <article key={index} className="ml-6 rounded-[22px] border border-border-base/80 bg-surface px-5 py-3.5 shadow-xs sm:ml-14"><p className="whitespace-pre-wrap text-[17px] leading-[1.65] text-ink">{turn.content}</p></article> : <article key={index} className="border-l-2 border-accent/50 py-1 pl-4"><p className="whitespace-pre-wrap text-[16px] leading-[1.85] text-ink-body">{turn.content}</p></article>)}<CoreAnswerCard answer={answer} loading={loading} onRequest={askAlternative} retryLabel="看看易經視角" expectedLens="diamond_sutra" />{answer && (alternative || alternativeLoading) && <CoreAnswerCard answer={alternative} loading={alternativeLoading} onRequest={askAlternative} retryLabel="再看一次易經視角" expectedLens="i_ching" />}</section>
