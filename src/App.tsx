...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (flow.state) {
      case 'HOME':
        return <HomeScreen onStartInput={text => flow.submitText(text)} onReview={flow.openReview} onOpenBackup={flow.openBackup} getTemporalCandidate={flow.getTemporalCandidate} onResolveTemporalDelta={flow.resolveTemporalDelta} getContinuityCandidate={flow.getContinuityCandidate} onResolveContinuity={flow.resolveContinuityProbe} onResumeContinuity={flow.resumeContinuityMoment} onSuppressContinuity={flow.suppressContinuityProbe} onDismissContinuity={flow.dismissContinuityProbe} dockedMoment={flow.dockedMoment} onOpenChat={flow.openChat} onDismissDockedMoment={flow.dismissDockedMoment} requestPresentReply={flow.requestPresentReply} persistenceState={flow.persistenceState} getTodayAnchorStats={flow.getTodayAnchorStats} onRecordAnchorEvent={flow.recordAnchorEvent} />;
      case 'CHAT':
      ...
      case 'REVIEW':
        return <ReviewScreen onClose={flow.finish} getMoments={flow.getMoments} getSessions={flow.getSessions} onOpenSession={flow.openSession} canShowPatternMirror={flow.canShowPatternMirror} onRequestPatternMirror={flow.requestPatternMirror} onSettleItem={flow.settleItem} onUnsettleItem={flow.unsettleItem} onSettleAllStill={flow.settleAllStill} onDeleteItem={flow.deleteItem} onOpenBackup={flow.openBackup} onResolveTemporalDelta={flow.resolveTemporalDelta} />;
      case 'BACKUP':
