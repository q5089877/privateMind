...
export default function App() {
  ...
  const start = async (lens: PerspectiveId) => {
  ...
  const deleteSelectedConversations = () => {
    ...
    setHistory(current => {
      const next = current.filter(record => !selectedHistoryIds.includes(record.id));
      saveConversationHistory(next);
      return next;
    });
