...
export default function App() {
  ...
  const askAlternative = async () => {
    ...
    try {
      const result = await getCoreAnswer({ content: latest.content, recentContext: turns.map(turn => turn.content).slice(-3), preferredLens: 'tao_te_ching' });
  ...
  if (chat) return <main className="mx-auto min-h-screen w-full max-w-[680px] px-4 py-5 sm:px-8">
    ...
    <div className="mb-7 mt-4"><h1 className="text-[25px] font-medium tracking-[-0.04em] text-ink sm:text-[30px]">先看見你留下的問題</h1><p className="mt-2 text-[15px] leading-relaxed text-ink-secondary">金剛經先照見，道德經視角由你決定要不要看。</p></div>
    <section className="space-y-5">{turns.map((turn, index) => turn.role === 'user' ? <article key={index} className="ml-6 rounded-[22px] border border-border-base/80 bg-surface px-5 py-3.5 shadow-xs sm:ml-14"><p className="whitespace-pre-wrap text-[17px] leading-[1.65] text-ink">{turn.content}</p></article> : <article key={index} className="border-l-2 border-accent/50 py-1 pl-4"><p className="whitespace-pre-wrap text-[16px] leading-[1.85] text-ink-body">{turn.content}</p></article>)}<CoreAnswerCard answer={answer} loading={loading} failed={answerFailed} onRequest={answerFailed ? retryAnswer : askAlternative} retryLabel={answerFailed ? '重試' : alternativeFailed ? '重試道德經視角' : '看看道德經視角'} expectedLens="diamond_sutra" />{answer && (alternative || alternativeLoading) && <CoreAnswerCard answer={alternative} loading={alternativeLoading} failed={alternativeFailed} onRequest={askAlternative} retryLabel="重試道德經視角" expectedLens="tao_te_ching" />}</section>
