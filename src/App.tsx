...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (flow.state) {
      case 'HOME':
        return <HomeScreen onStartInput={text => flow.submitText(text)} onReview={flow.openReview} onOpenBackup={flow.openBackup} onBeginLanding={flow.beginLandingFromMoment} dockedMoment={flow.dockedMoment} onOpenChat={flow.openChat} onDismissDockedMoment={flow.dismissDockedMoment} persistenceState={flow.persistenceState} getTodayAnchorStats={flow.getTodayAnchorStats} onRecordAnchorEvent={flow.recordAnchorEvent} />;
      case 'CHAT':
