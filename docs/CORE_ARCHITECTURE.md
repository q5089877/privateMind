...
- **先停在這句（focus）**：停在一個具體詞句，指出其中值得細看的位置；不急著替它找原因。
- **拆成兩邊看（contrast）**：把原話裡兩個能並排看的部分放在一起，提出可觀察的拉扯；沒有足夠原話時，誠實地只分辨同一句的兩個面向。
- **從變化看（reframe）**：從原文已經出現的另一個位置，提出不同但仍可驗證的觀看方式；不能把推測說成事實或因果。
- **留住未答處（open）**：標出今天仍沒有答案、卻值得留下的位置；不是空泛安慰，也不是催促使用者繼續回答。
